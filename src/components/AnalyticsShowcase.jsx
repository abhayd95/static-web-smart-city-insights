    ...
    >
      <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-8 h-full">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-2xl font-bold text-gray-900 max-w-sm">{data.title}</h3>
          <div
            ref={counterRef}
            className={`text-4xl font-extrabold bg-gradient-to-r ${data.color} text-transparent bg-clip-text`}
          >
            {value}
            <span className="text-2xl align-top ml-1">{data.suffix}</span>
          </div>
        </div>
        <p className="text-gray-600 text-sm mb-8 leading-relaxed">{data.description}</p>

        <div className="grid grid-cols-3 gap-4">
          {data.segments.map((segment, idx) => (
            <div key={idx} className="flex flex-col">
              <div className="flex-1 bg-gray-100 rounded-lg p-2">
                <div className="h-24 bg-gradient-to-t from-gray-200 to-gray-100 rounded-md overflow-hidden relative">
                  <div
  ...
  return (
    <ScrollReveal animation="fade-in" delay={delay}>
      <div className="bg-gray-50 rounded-xl p-4 flex items-center justify-between">
        ...
        <span ref={ref} className="text-2xl font-bold text-gold-600">
...
const HeatmapCard = () => (
  <ScrollReveal animation="fade-in" delay={3}>
    <div className="bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 rounded-2xl p-8 text-white h-full">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {heatmapData.map((item, index) => (
          <ScrollReveal key={item.label} animation={index % 2 === 0 ? 'slide-left' : 'slide-right'} delay={index % 3}>
